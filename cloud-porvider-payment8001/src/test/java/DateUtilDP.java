...
public class DateUtilDP {
    private static final String C_thisMonth_CODE = "0";
    private static final String C_lastMonth_CODE = "1";
    private static final String C_thisWeek_CODE = "2";
    private static final String C_thisSeason_CODE = "3";

    public static void main(String[] args) {
        System.out.println(GetDateWhere("sysdate", "0"));
    }


    public static String GetDateWhere(String StaTime, String type) {
        ImDateWhere imDateWhere;
        if (C_thisMonth_CODE.equals(type)) {
            imDateWhere = new C_thisMonth();
        } else if (C_lastMonth_CODE.equals(type)) {
            imDateWhere = new C_lastMonth();
        } else if (C_lastMonth_CODE.equals(type)) {
            imDateWhere = new C_thisWeek();
        } else if (C_thisWeek_CODE.equals(type)) {
            imDateWhere = new C_thisSeason();
        } else if (C_thisSeason_CODE.equals("4")) {
...
class C_thisMonth implements ImDateWhere {
    ...
    public String DateWhereShow(String StaTime) {
        ...
        return "TO_CHAR(" + StaTime + ",'yyyy-mm-dd') BETWEEN " + monthEnd + " and " + monthStart;
    }
